...
class Question(UUIDPK, db.Model):
    ...
    command_verb_id: Mapped[uuid.UUID | None] = mapped_column(
        Uuid(), ForeignKey("command_verbs.id", ondelete="SET NULL"), nullable=True
    )
    ...
    mark_scheme: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    explanation: Mapped[str | None] = mapped_column(Text, nullable=True)
    performance_outcome_id: Mapped[uuid.UUID | None] = mapped_column(
        Uuid(), ForeignKey("performance_outcomes.id", ondelete="SET NULL"), nullable=True
    )
    assessment_objective_id: Mapped[uuid.UUID | None] = mapped_column(
        Uuid(), ForeignKey("assessment_objectives.id", ondelete="SET NULL"), nullable=True
    )
